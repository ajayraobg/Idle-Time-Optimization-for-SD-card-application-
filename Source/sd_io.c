...
//Modified by Ajay Rao for adding RTOS elements in SD functions for better CPU utilization.
...
SDRESULTS SD_Init(SD_DEV *dev)
{
		...
		for(init_trys=0; ((init_trys!=SD_INIT_TRYS)&&(!ct)); init_trys++)
    {
        ...
        dev->mount = FALSE;
				...
				//osDelayUntil(tick + 2000);
        //SPI_Timer_On(500);
        while ((__SD_Send_Cmd(CMD0, 0) != 1)&&((tick2 = osKernelGetTickCount() )<tick1+2000)) {
				}
	      //SPI_Timer_Off();
        // Idle state
        if (__SD_Send_Cmd(CMD0, 0) == 1) {                      
            ...
            if (__SD_Send_Cmd(CMD8, 0x1AA) == 1) {
                ...
                if ((ocr[2] == 0x01)&&(ocr[3] == 0xAA))
                {
                    ...
                    //SPI_Timer_On(1000);
										tick1 = osKernelGetTickCount();
                    while (((tick2 = osKernelGetTickCount())<tick1+4000)&&(__SD_Send_Cmd(ACMD41, 1UL << 30))) {
										}
                    //SPI_Timer_Off(); 
                    // CCS in the OCR? 
                ...
                //SPI_Timer_On(250);
								tick1 = osKernelGetTickCount();
                while(((tick2 = osKernelGetTickCount())< tick1+1000)&&(__SD_Send_Cmd(cmd, 0))) {
								}
                //SPI_Timer_Off();
